...
import type { AiShadowAuditRecord, AiShadowValidationReport, EvidenceIntakeApplyResult, EvidenceIntakeSession, IntakeEvaluationReport, RawDocument, RawDocumentKind, ReviewDecision } from '@/features/intake/types/intake';
...
export class IntakeArtifactRepository {
  ...
  readEvidenceNodes(): EvidenceNode[] {
    ...
    for (const relativeDirectory of ['data/sample_evidence', 'data/live_evidence']) {
      ...
      for (const file of readdirSync(directory).filter((item) => item.endsWith('.yaml') || item.endsWith('.yml')).sort()) {
        try {
          const value = readGenericArtifact(resolve(directory, file))! as EvidenceNode | EvidenceNode[];
  ...
  writeIntakeSession(session: EvidenceIntakeSession, workbenchHtml: string): void {
    writeGenericArtifact(resolve(this.repoRoot, LATEST_INTAKE_SESSION_PATH), session);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_raw_document.json`, session.raw_document);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_chunks.json`, session.chunks);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_candidates.json`, session.candidates);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_provenance.json`, session.provenance_records);
    writeGenericTextArtifact(resolve(this.repoRoot, LATEST_REVIEW_DECISIONS_PATH), stringify(session.review_template));
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_workbench.html`, workbenchHtml);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${session.session_id}.json`, session);
  ...
  writeAiShadowResult(session: EvidenceIntakeSession, audit: unknown): void {
    writeGenericArtifact(resolve(this.repoRoot, LATEST_INTAKE_SESSION_PATH), session);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_ai_shadow_candidates.json`, session.ai_shadow_candidates ?? []);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_candidate_comparisons.json`, session.candidate_comparisons ?? []);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_ai_shadow_audit.json`, audit);
    writeGenericArtifact(resolve(this.repoRoot, `${INTAKE_OUTPUT_DIR}/history/${(audit as AiShadowAuditRecord).audit_id ?? `ai_shadow_${session.session_id}`}.json`), audit);
  ...
  writeAiShadowValidationReport(report: AiShadowValidationReport): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_ai_shadow_validation_report.json`, report);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_ai_shadow_validation_report.md`, renderAiShadowValidationMarkdown(report));
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${report.report_id}.json`, report);
    this.appendAudit({ event: 'ai_shadow_validation_report', ...report });
  }

  writeAiShadowCorpusReport(report: AiShadowValidationReport): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_real_ai_shadow_evaluation.json`, report);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_real_ai_shadow_evaluation.md`, renderAiShadowValidationMarkdown(report));
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${report.report_id}_corpus.json`, report);
    this.appendAudit({ event: 'ai_shadow_corpus_evaluation', ...report });
  }

  writeIntakeAgentBundle(bundle: IntakeAgentReviewBundle): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_agent_candidates.json`, bundle.candidates);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_agent_verification.json`, bundle.verification);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_agent_audit.json`, bundle.audit);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_agent_review.json`, bundle);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_agent_review.md`, renderAgentReviewMarkdown(bundle));
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${bundle.audit.audit_id}.json`, bundle);
  ...
  readLatestSession(): EvidenceIntakeSession {
    return readGenericArtifact(resolve(this.repoRoot, LATEST_INTAKE_SESSION_PATH))! as EvidenceIntakeSession;
  }

  readLatestAgentBundle(): IntakeAgentReviewBundle | null {
    const path = `${INTAKE_OUTPUT_DIR}/latest_agent_review.json`;
    if (!existsSync(path)) return null;
    try {
      return readGenericArtifact(path)! as IntakeAgentReviewBundle;
    } catch {
      return null;
    }
  }
  ...
  readLearningProfile(): IntakeLearningProfile | null {
    const path = `${INTAKE_OUTPUT_DIR}/latest_learning_profile.json`;
    if (!existsSync(path)) return null;
    return readGenericArtifact(path)! as IntakeLearningProfile;
  }

  writeLearningProfile(profile: IntakeLearningProfile): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_learning_profile.json`, profile);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_learning_profile.md`, renderLearningProfileMarkdown(profile));
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${profile.profile_id}.json`, profile);
  ...
  readPreviousLearningProfile(currentProfileId: string): IntakeLearningProfile | null {
    const directory = `${INTAKE_OUTPUT_DIR}/history`;
    if (!existsSync(directory)) return null;
    const profiles = readdirSync(directory)
      .filter((file) => file.endsWith('.json'))
      .flatMap((file) => {
        try {
          const value = readGenericArtifact(resolve(directory, file))! as Partial<IntakeLearningProfile>;
  ...
  readAiShadowValidationReport(): AiShadowValidationReport | null {
    const path = `${INTAKE_OUTPUT_DIR}/latest_ai_shadow_validation_report.json`;
    if (!existsSync(path)) return null;
    return readGenericArtifact(path)! as AiShadowValidationReport;
  }

  writeLearningCycle(cycle: IntakeLearningCycle): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_learning_cycle.json`, cycle);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_active_learning_queue.json`, cycle.active_learning_queue);
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_learning_proposals.json`, cycle.proposals);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_learning_cycle.md`, renderLearningCycleMarkdown(cycle));
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${cycle.cycle_id}.json`, cycle);
    this.appendAudit({
  ...
  readReviewDecisions(file = LATEST_REVIEW_DECISIONS_PATH): ReviewDecision[] {
    const value = readGenericArtifact(resolve(this.repoRoot, file))! as ReviewDecision | ReviewDecision[];
    return Array.isArray(value) ? value : [value];
  }

  readApplyResult(): EvidenceIntakeApplyResult | null {
    const path = `${INTAKE_OUTPUT_DIR}/latest_apply_result.json`;
    if (!existsSync(path)) return null;
    return readGenericArtifact(path)! as EvidenceIntakeApplyResult;
  }

  readLatestEvaluation(): IntakeEvaluationReport {
    return readGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_evaluation.json`)! as IntakeEvaluationReport;
  }

  writeEvidenceDraft(drafts: EvidenceImportDraft[]): string {
    writeGenericTextArtifact(resolve(this.repoRoot, REVIEWED_EVIDENCE_DRAFT_PATH), stringify(drafts));
    return REVIEWED_EVIDENCE_DRAFT_PATH;
  }

  writeApplyResult(result: EvidenceIntakeApplyResult): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_apply_result.json`, result);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_apply_result.md`, renderApplyMarkdown(result));
    const version = result.generated_at.replace(/[-:.TZ]/g, '').slice(0, 17);
    const retry = result.pipeline_retry_count ?? 0;
    writeGenericArtifact(
      `${INTAKE_OUTPUT_DIR}/history/apply_${result.session_id}_${version}_r${retry}.json`,
      result,
    );
  ...
  writeIntakeEvaluation(report: IntakeEvaluationReport): void {
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/latest_evaluation.json`, report);
    writeGenericTextArtifact(`${INTAKE_OUTPUT_DIR}/latest_evaluation.md`, renderEvaluationMarkdown(report));
    writeGenericArtifact(`${INTAKE_OUTPUT_DIR}/history/${report.evaluation_id}.json`, report);
  ...
  readStageChangeSummary(): unknown {
    ...
    for (const relativePath of ['outputs/operator_runs/latest_weekly_brief.json', 'outputs/reports/weekly_brief.json']) {
      ...
      try {
        const weekly = readGenericArtifact(path)! as { stage_change_summary?: unknown };
...
function parseDocument(path: string, kind: RawDocumentKind): string {
  if (kind === 'txt' || kind === 'markdown') return (readGenericTextArtifact(path) ?? "");
  if (kind === 'html') return stripHtml((readGenericTextArtifact(path) ?? ""));
