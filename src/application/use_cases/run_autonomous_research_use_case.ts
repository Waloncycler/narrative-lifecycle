...
export interface RunAutonomousResearchUseCaseDeps {
  ...
  validateDrafts(input: { drafts: import('../../types/evidence_import').EvidenceImportDraft[]; sourceFile: string; generatedAt: string }): EvidenceValidationReport;
  normalizeDrafts(input: { drafts: import('../../types/evidence_import').EvidenceImportDraft[]; sourceFile: string; importedAt: string }): NormalizedEvidenceImport[];
  ...
  validateDiff(diff: import('../../types/diff').StageDiff): void;
  validateWeeklyBrief(brief: import('../../types/report').WeeklyBrief): void;
}
