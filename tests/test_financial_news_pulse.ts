...
const policy = {
  ...
  financial_news_domains: ['wallstreetcn.com', 'cls.cn', 'finance.sina.com.cn', 'cn.wsj.com'],
...
describe('financial news pulse', () => {
  ...
  it('allows media pages only as bounded deep-probe inputs', () => {
    ...
    expect(isFinancialNewsProbe(media, policy)).toBe(true);
    expect(isFinancialNewsProbe({ source_class: 'secondary', source_domain: 'finance.sina.com.cn' }, policy)).toBe(true);
    expect(isFinancialNewsProbe({ source_class: 'secondary', source_domain: 'cn.wsj.com' }, policy)).toBe(true);
    expect(isFinancialNewsProbe(ordinary, policy)).toBe(false);
    const items = [
    ...
    expect(selectSourceRetrievalTargets(report(items), 4, 2, policy).map((item) => item.triage_id)).toEqual(['media']);
  });
});
