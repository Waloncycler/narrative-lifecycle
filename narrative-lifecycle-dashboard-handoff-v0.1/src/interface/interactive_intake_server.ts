...
import {
  ...
  renderTopicDetail,
  renderTopics,
} from './narrative_monitor_renderer';
  ...
  return createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'PUT' && pathname === '/api/agent/scheduler-config') {
        const body = await readJson<import('../types/research_agent').ResearchAgentSchedulerConfig>(request);
      ...
      if (request.method === 'POST' && pathname === '/api/agent/confirm-all-chain-entries') {
        ...
        if (existsSync(chainPath)) {
          const entries = JSON.parse(readFileSync(chainPath, 'utf8')) as import('../types/evidence_chain').EvidenceChainEntry[];
        ...
        if (existsSync(proposalPath)) {
          const proposals = JSON.parse(readFileSync(proposalPath, 'utf8')) as import('../types/topic_discovery').TopicDiscoveryProposal[];
...
function sanitizeSchedulerConfig(body: import('../types/research_agent').ResearchAgentSchedulerConfig): import('../types/research_agent').ResearchAgentSchedulerConfig {
...
function readMonitor(repoRoot: string, useCases?: ProductCoreUseCases) {
  ...
  const learningCycle = readJsonFile<import('../types/intake_learning_cycle').IntakeLearningCycle>(repoRoot, 'outputs/intake/latest_learning_cycle.json');
  ...
  const graphPromotion = readJsonFile<import('../types/narrative_graph_promotion').NarrativeGraphPromotionReport>(repoRoot, 'outputs/autonomy/latest_narrative_graph_promotion.json');
  const webResearch = readJsonFile<import('../types/web_research').WebResearchReport>(repoRoot, 'outputs/research/latest_web_research.json');
  ...
  const topicDiscoveryProposals = readJsonFile<import('../types/topic_discovery').TopicDiscoveryProposal[]>(repoRoot, 'outputs/intake/latest_topic_discovery_proposals.json') ?? [];
  const evidenceChain = readJsonFile<import('../types/evidence_chain').EvidenceChainEntry[]>(repoRoot, 'outputs/intake/latest_evidence_chain.json') ?? [];
  ...
  const researchAgent = useCases
    ? {
        ...
        loop_running: useCases.researchAgentScheduler.running,
        next_daily_run: useCases.researchAgentScheduler.nextDailyRun(),
        last_run: readJsonFile<import('../types/research_agent').ResearchAgentRunManifest>(repoRoot, 'outputs/research_agent/latest_run.json'),
        run_history: useCases.researchAgentRepository.listRunManifests(),
        evolution: useCases.researchAgentRepository.readEvolutionLedger(),
        scheduler: useCases.researchAgentRepository.readSchedulerConfig(),
