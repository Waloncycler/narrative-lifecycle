...
import { RunWebResearchUseCase } from '../src/application/use_cases/run_web_research_use_case';
...
describe('Web research discovery boundary', () => {
  ...
  it('writes a schema-valid unconfigured result without pretending that search became evidence', async () => {
    ...
    const useCase = new RunWebResearchUseCase({
      ...
      config: () => ({ provider: 'disabled', endpoint: null, api_key: null, timeout_ms: 1000, max_results_per_query: 5 }),
  ...
  it('is schema-valid and remains unable to alter Stage/Score', async () => {
    const useCase = new RunWebResearchUseCase({
      ...
      config: () => ({ provider: 'gdelt', endpoint: 'https://example.test/search', api_key: null, timeout_ms: 1000, max_results_per_query: 5 }),
      readRegistry: () => registry,
